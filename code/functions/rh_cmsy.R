...
calc_r_priors <- function(key){
  ...
  r_prior_fl <- r_prior(key$species)
...
# Robin-Hood cMSY
# data <- sdata; key <- key; npairs <- 5000
rh_cmsy <- function(data, key, npairs=10000){
  ...
  if(r_prior_method=="resilience"){
    rs <- runif(npairs, r_prior$r_lo, r_prior$r_hi)
  }
  ks <- runif(npairs, k_prior$k_lo, k_prior$k_hi)
  ...
  # Check number viables
  if(nviable==0){stop("No viable trajectories found.")}
...
plot_rh_cmsy <- function(output, true, savedir){
  ...
  if(r_prior_use=="FishLife"){
    ln_r_mu <- output$r_prior$ln_r_mu
    ln_r_sd <- output$r_prior$ln_r_sd
